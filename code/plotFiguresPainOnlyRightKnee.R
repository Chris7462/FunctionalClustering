...
savePath = 'fclustResult/PainRightKnee/'
...
Data = Data[4] # only pain data, right knee
data = cbind(Data[[1]]) 
...
saveRDS(dfs, paste(RDSFigsPath, 'PainR_Sil.rds', sep=''))
...
for (i in 1:1) {
  ...
  saveRDS(dft, paste(RDSFigsPath, 'PainR_Traj.rds', sep=''))
}
dev.off()
...
year = 1
  ...
  write.table(testProb, file = "predResult/PainRightKnee/testProb.txt", sep = "\t",
              row.names = FALSE, col.names = FALSE)
  
  write.table(testProbPred[,,lambdaIdx], file = "predResult/PainRightKnee/testProbPred.txt", sep = "\t",
              row.names = FALSE, col.names = FALSE)
  ...
  saveRDS(df, paste(RDSFigsPath, 'PainR_AUC21_',year,'.rds', sep=''))
  ...
  names(df)[1]<-paste("Specificity")
  names(df)[2]<-paste("Sensitivity")
  saveRDS(df, paste(RDSFigsPath, 'PainR_AUC31_',year,'.rds', sep=''))
  ...
  names(df)[1]<-paste("Specificity")
  names(df)[2]<-paste("Sensitivity")
  saveRDS(df, paste(RDSFigsPath, 'PainR_AUC32_',year,'.rds', sep=''))
