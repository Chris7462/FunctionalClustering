...
fclustPath = savePath
predPath = 'predResult/PainLeftKnee/'
...
dev.off()
dfs <-data.frame(G[-1], fclustSil)
names(dfs)[1]<-paste("Nclust")
saveRDS(dfs, paste(RDSFigsPath, 'PainL_Sil.rds', sep=''))
...
for (i in 1:1) {
  ...
  Mean3 <- res$meancurves[idx,3]
  dft <- data.frame(Mean1, Mean2, Mean3)
  saveRDS(dft, paste(RDSFigsPath, 'PainL_Traj.rds', sep=''))
}
dev.off()
...
year=1
  ...
  write.table(testProb, file = "predResult/PainLeftKnee/testProb.txt", sep = "\t",
              row.names = FALSE, col.names = FALSE)
  
  write.table(testProbPred[,,lambdaIdx], file = "predResult/PainLeftKnee/testProbPred.txt", sep = "\t",
              row.names = FALSE, col.names = FALSE)
  ...
  df21 <- data.frame(1-rocCurve$specificities, rocCurve$sensitivities)
  names(df21)[1]<-paste("Specificity")
  names(df21)[2]<-paste("Sensitivity")
  saveRDS(df21, paste(RDSFigsPath, 'PainL_AUC21_',year,'.rds', sep=''))
  ...
  rocCurve = roc(cases = testProbPred[,caseID,lambdaIdx][caseIdx], 
                 controls = testProbPred[,caseID,lambdaIdx][!caseIdx], plot=F)
  df31 <- data.frame(1-rocCurve$specificities, rocCurve$sensitivities)
  names(df31)[1]<-paste("Specificity")
  names(df31)[2]<-paste("Sensitivity")
  saveRDS(df31, paste(RDSFigsPath, 'PainL_AUC31_',year,'.rds', sep=''))
  ...
  rocCurve = roc(cases = testProbPred[,caseID,lambdaIdx][caseIdx], 
                 controls = testProbPred[,caseID,lambdaIdx][!caseIdx], plot=F)
  df32 <- data.frame(1-rocCurve$specificities, rocCurve$sensitivities)
  names(df32)[1]<-paste("Specificity")
  names(df32)[2]<-paste("Sensitivity")
  saveRDS(df32, paste(RDSFigsPath, 'PainL_AUC32_',year,'.rds', sep=''))
