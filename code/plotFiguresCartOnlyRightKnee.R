...
savePath = 'fclustResult/CartRightKnee/'
Data = readRDS('fclustResult/Data.rds')

fclustPath = savePath
predPath = 'predResult/CartRightKnee/'
...
length(availIdx)
cartilage = cartilage[availIdx, 1:8+8]
dim(cartilage)
# remove years with no observations
cartilage1 = cartilage[, -c(6,8)]

charac = readRDS('predResult/charac.rds')
p = ncol(charac)

### functional clustering
Data = Data[2] # only cartilage data, right knee
data = cbind(Data[[1]]) 
...
saveRDS(dfs, paste(RDSFigsPath, 'CartR_Sil.rds', sep=''))
...
for (i in 1:1) {
  ...
  saveRDS(dft, paste(RDSFigsPath, 'CartR_Traj.rds', sep=''))
}
dev.off()
...
for (year in 1:6) {
  ...
  saveRDS(df, paste(RDSFigsPath, 'CartR_AUC',year,'.rds', sep=''))
  
  write.table(testProb, file = "predResult/CartRightKnee/testProb.txt", sep = "\t",
              row.names = FALSE, col.names = FALSE)
  
  write.table(testProbPred[,lambdaIdx], file = "predResult/CartRightKnee/testProbPred.txt", sep = "\t",
              row.names = FALSE, col.names = FALSE)
