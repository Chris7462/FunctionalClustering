...
savePath = 'fclustResult/CartLeftKnee/'
Data = readRDS('fclustResult/Data.rds')

fclustPath = savePath
predPath = 'predResult/CartLeftKnee/'
...
length(availIdx)
cartilage = cartilage[availIdx, 1:8]
dim(cartilage)
...
Data = Data[1] # only cartilage data, left knee
data = cbind(Data[[1]]) 
...
saveRDS(dfs, paste(RDSFigsPath, 'CartL_Sil.rds', sep=''))
...
for (i in 1:1) {
  ...
  saveRDS(dft, paste(RDSFigsPath, 'CartL_Traj.rds', sep=''))
}
dev.off()
...
for (year in 1:6) {
  ...
  saveRDS(df, paste(RDSFigsPath, 'CartL_AUC',year,'.rds', sep=''))

  write.table(testProb, file = "predResult/CartLeftKnee/testProb.txt", sep = "\t",
              row.names = FALSE, col.names = FALSE)
  
  write.table(testProbPred[,lambdaIdx], file = "predResult/CartLeftKnee/testProbPred.txt", sep = "\t",
              row.names = FALSE, col.names = FALSE)
