library(Matrix)
"fitfclust.multicurve" <-
  function(data, q = 5, h = 1, p = 5,
             ...
             0.01, grid = seq(0, 1, length = 100), hard = F, plot= F,trace=F)
  {
    ...
    library(splines)
    # data is a list of list(x=x,curve=curve,timeindex=timeindex)
    J = length(data)
    print("Initialize the parameters")
    initfit <- fclustinit.multicurve(data = data, pert = pert, grid = grid, h = h,
                          p = p, q = q, K = K)
    ...
    S <- initfit$S
    FullS <- initfit$FullS
    FFullS = lapply(1:J, function(j) {FullS})
    ...
    # sigma  has converged.
    print("Main loop. Iterate between M and E steps until convergence")
    while(max(abs(sigma.old - sigma.new)/sigma.new) > tol & (ind <= maxit)) {
      parameters <- fclustMstep.multicurve(parameters, data, vars, S, tol, p, hard)
      vars <- fclustEstep.multicurve(parameters, data, vars, S, hard)
      sigma.old <- sigma.new
      ...
      if(plot){
        cluster.mean <- matrix(0,K,dim(FFullS)[1])
        for(k in 1:K)
          cluster.mean[k,] <- FFullS %*% (parameters$lambda.zero + parameters$Lambda %*% parameters$alpha[k,  ])
        par(mfrow = c(1,J))
        TT = length(grid)
        for (j in 1:J) {
          idx = 1:TT + (j-1)*TT
          plot(grid,grid,ylim=range(cluster.mean[,idx]),type='n',ylab="Cluster Mean", main = paste('Curve',j))
          for(k in 1:K)
            lines(grid,cluster.mean[k,idx], col = 4, lwd = 2)
        }
      }
      ind <- ind + 1
    }
    # Enforce parameter constraint (7)
    cfit <- fclustconst.multicurve(data, parameters, vars, FFullS)
    list(data=data,parameters = cfit$parameters, vars = cfit$vars, FullS = FullS,grid=grid)
  }

# change the spline basis to polynomials
"fclustinit.multicurve" <-
  function(data, pert = 0, grid = seq(0.01, 1, length = 100), h = 1, p = 2, q = 5,K = K){
    ...
    FullS <- cbind(1, ns(grid, df = (q - 1))) #poly(grid, degree=q)
    FullS <- svd(FullS)$u
    J = length(data)
    S = rep(list(), J)
    Points = c()
    ...
    for (j in 1:J) {
      dataj = data[[j]]
      S[[j]] <- FullS[dataj$timeindex,  ]
      # Compute initial estimate of basis coefficients.
      points <- matrix(0,N,q)
      for (i in 1:N){
        Si <- S[[j]][dataj$curve==i,]
        xi <- dataj$x[dataj$curve==i]
        
        if (is.null(dim(Si))) Si <- t(Si)
    ...
    classmean <- matrix(0,K,q*J)
    for (k in 1:K)
      classmean[k,] <- apply(Points[class==k,],2,mean)

    # Initialize lambda.zero, Lambda and alpha as defined in paper.
    lambda.zero = apply(classmean, 2, mean) # q*J
    Lambda <- as.matrix(svd(scale(classmean, scale = F))$v[, 1:h]) # (q*J, h)
    alpha <- scale(classmean, scale = F)%*%Lambda # (K, h)
    # Calculate estimates for gamma and gprod.
    gamma <- t(t(Points) - lambda.zero - (Lambda %*% t(alpha[class,  ])))
    gprod <- NULL
    for(i in 1:N) gprod <- cbind(gprod, (gamma[i,  ]) %*% t(gamma[i,  ])) # (q*J, N*q*J)
    gamma <- array(gamma[, rep(1:(q*J), rep(K, (q*J)))], c(N, K, q*J))
    gcov <- matrix(0, q*J, N * q*J)
...
"fclustMstep.multicurve" <-
  function(parameters, data, vars, S, tol, p, hard)
  {
    ...
    ind <- matrix(rep(c(rep(c(1, rep(0, q*J - 1)), N), 0), q*J)[1:(N*(q*J)^2)], N * q*J, q*J)
    ...
    loopnumber <- 1
    lambda.zero = rep(NA, q*J)
    while((max(abs(sigma.old[1] - sigma[1])/sigma[1]) > tol) & (loopnumber <10)){
      #    cat(loopnumber)
      sigma.old <- sigma
      # Calculate lambda.zero.
      #print("Calculate lambda zero")
      for (j in 1:J) {
        curve <- data[[j]]$curve
        n <- length(curve)
        idx = 1:q + (j-1)*q
        gamma.pi <- diag(S[[j]] %*% t(apply(gamma[,,idx] * as.vector(piigivej),
                                       c(1, 3), sum)[curve,  ]))
        alpha.pi <- t(matrix(apply(alpha, 2, function(x, piigivej,K)
        {rep(1, K) %*% (piigivej * x)}
        , t(piigivej), K), N, h)[curve,  ])
        lambda.zero[idx] <- solve(t(S[[j]]) %*% S[[j]]) %*% t(S[[j]]) %*% (data[[j]]$x - diag(
          S[[j]] %*% Lambda[idx,] %*% alpha.pi) - gamma.pi)
      }
      ...
      for(i in 1.:K) {
        ...
        for (j in 1:J) {
          idx = 1:q + (j-1)*q
          curve = data[[j]]$curve
          x <- data[[j]]$x - S[[j]] %*% lambda.zero[idx]
          S.Lam <- S[[j]] %*% Lambda[idx,]
          S.Lam.pi <- S.Lam * piigivej[curve, i]
          aa = aa + t(S.Lam.pi) %*% S.Lam / (sigma[j])
          bb = bb + t(S.Lam.pi) %*% (x - diag(S[[j]] %*% t(gamma[curve, i,  idx]))) / (sigma[j])
        }
      ...
      #print("Calculate Lambda given alpha")
      for(m in 1:h) {
        for (j in 1:J) {
          idx = 1:q + (j-1)*q
          curve = data[[j]]$curve
          ...
          S.Lambda <- t(S[[j]] %*% Lambda[idx,])
          if(h != 1) {
            temp <- NULL
            for(i in 1:K) {
              temp <- cbind(temp, as.vector(rep(1, h - 1) %*% matrix((S.Lambda *
                                                                        alpha[i,  ])[ - m,  ], h - 1,dim(S[[j]])[1])) * alpha[i, m])
            }
            otherLambda <- (temp * piigivej[curve,  ])%*%rep(1, K)
          }
          else otherLambda <- 0
          gamma.pi.alpha <- apply(gamma[,,idx] * as.vector(piigivej) *
                                    rep(alpha[, m], rep(N, K)), c(1, 3), sum)[curve,  ]
          Lambda[idx, m] <- solve(t(S[[j]] * pi.alphasq) %*% S[[j]]) %*% t(S[[j]]) %*%
            (x * pi.alpha - otherLambda - (S[[j]] *gamma.pi.alpha) %*% rep(1, q))
        }
      }
      # Calculate sigma 
      for (j in 1:J) {
        ...
        for(i2 in 1:K) {
          # cat(i2)
          vect1 <- x - S[[j]] %*% Lambda[idx,] %*% alpha[
            i2,  ] - (S[[j]] * gamma[curve, i2,  idx]) %*% rep(1, q)
          econtrib <- econtrib + t(piigivej[curve,i2] * vect1) %*% vect1
        }
        
        individualTraceVal <- sapply(1:N, function(ii) {
          idx1 <- which(curve==ii)
          si <- S[[j]][idx1,]
          if (is.null(dim(si))) si <- t(si)
          idx2 <- (ii-1)*q*J + 1:(q*J)
          return(sum(diag(si %*% gcov[,idx2][idx,idx] %*% t(si))))
        })
        traceVal <- sum(individualTraceVal)
        sigma[j] <- as.vector((econtrib + traceVal)/n)
      }
...
"fclustEstep.multicurve" <-
  function(parameters, data, vars, S, hard)
  {
    ...
    N <- dim(vars$gamma)[1]
    K <- dim(vars$gamma)[2]
    q <- dim(vars$gamma)[3]/J
    Gamma <- par$Gamma
    Lambda.alpha <- par$lambda.zero + par$Lambda %*% t(par$alpha)
    vars$gprod <- vars$gcov <- NULL
    for(j in 1:N) {
      # Calculate expected value of gamma.
      temp = lapply(1:J, function(jj) {
        res = S[[jj]][data[[jj]]$curve==j,]
        if (is.null(dim(res))) res = t(res)
        res
        })
      nj = sapply(temp, nrow)
      snj = sum(nj)
      #Sj <- S[data$curve == j,  ]
      #if (is.null(dim(Sj))) Sj <- t(Sj)
      #nj <- sum(data$curve == j)
      #invvar <- diag(1/rep(par$sigma, nj), nrow=nj, ncol=nj)
      Sj = as.matrix(bdiag(temp))
      invvar = diag(1/rep(par$sigma, nj), nrow = snj, ncol = snj)
      Cgamma <- Gamma - Gamma %*% t(Sj) %*% solve(diag(snj) + invvar %*% Sj %*%
                                                    Gamma %*% t(Sj)) %*% invvar %*% Sj %*% Gamma
      Y = lapply(data, function(y) {y$x[y$curve==j]})
      Y = unlist(Y)
      centx <- Y - Sj %*% Lambda.alpha
      vars$gamma[j,  ,  ] <- t(Cgamma %*% t(Sj) %*% invvar %*% centx)
      # Calculate pi i given j.
      covx <- Sj %*% par$Gamma %*% t(Sj) + solve(invvar)
      temp = - diag(t(centx) %*% solve(covx) %*% centx)/2
      temp = scale(temp, scale=F, center=T)
      d <- exp(temp) * par$pi
      ...
      vars$gprod <- cbind(vars$gprod, t(matrix(vars$gamma[j,  ,  ],
                                               K, q*J)) %*% (matrix(vars$gamma[j,  ,  ], K, q*J) * vars$
                                                             piigivej[j,  ]) + Cgamma)
...
"fclustconst.multicurve" <-
  function(data, parameters, vars, S)
  {
    ...
    J = length(data)
    TT = dim(S)[1]/J
    A <- t(S) %*% solve(diag(rep(par$sigma, rep(TT,J))) + S %*% par$Gamma %*%
                          t(S)) %*% S
    svdA <- svd(A)
...
"nummax" <-
  function(X)
  {
    # find maximum and its index for each row
    ind <- rep(1, dim(X)[1])
    m <- X[, 1]
    if(dim(X)[2] > 1)
      for(i in 2:dim(X)[2]) {
  ...
  function(fit,data=NULL,reweight=F)
  {
    ...
    for(i in 1:N) {
      temp = lapply(1:J, function(j) {
        res = FullS[data[[j]]$time[data[[j]]$curve==i], ]
        if (is.null(dim(res))) res = t(res)
        res
      })
      nj = sapply(temp, nrow)
      Sij = as.matrix(bdiag(temp))
      xij <- lapply(data, function(y) {y$x[y$curve == i]})
      xij = unlist(xij)
      n <- length(xij)
      Sigma <- diag(rep(par$sigma, nj)) + Sij %*% par$Gamma %*% t(Sij)
...
"fclust.curvepred.multicurve" <-
  function(fit, data=NULL, index=NULL, tau = 0.95, tau1 = 0.975)
  {
    if (is.null(data))
      data <- fit$data
    if (is.null(index))
      index <- 1:length(table(data[[1]]$curve))
    J = length(data)
    tau2 <- tau/tau1
    sigma <- fit$par$sigma
    Gamma <- fit$par$Gamma
    ...
    FFullS = as.matrix(bdiag(FFullS))
    N <- length(index)
    upci <-lowci <- uppi <- lowpi <- gpred <- matrix(0,N,nrow(S)*J)
    etapred <- matrix(0,N,ncol(S)*J)
    ind <- 1
    Lambda.alpha <- lambda.zero + Lambda %*% t(alpha)
    for (i in index){
      y <- lapply(data, function(yy) {yy$x[yy$curve == i]})
      y = unlist(y)
      #y <- data$x[data$curve == i]
      temp = lapply(1:J, function(j) {
        res = S[data[[j]]$time[data[[j]]$curve==i], ]
        if (is.null(dim(res))) res = t(res)
        res
      })
      nij = sapply(temp, nrow)
      Si = as.matrix(bdiag(temp))
      #Si <- S[data$time[data$curve == i],  ]
      #if (is.null(dim(Si))) Si <- t(Si)
      ni <- dim(Si)[1]
      invvar <- diag(1/rep(sigma, nij), nrow=ni, ncol=ni)
      covx <- Si %*% Gamma %*% t(Si) + solve(invvar)
      centx <- y - Si %*% Lambda.alpha
      temp <- - diag(t(centx) %*% solve(covx) %*% centx)/2
      temp <- scale(temp, scale=F, center=T)
      d <- exp(temp) * fit$par$pi
      pi <- d/sum(d)
      K <- length(pi)
      mu <- lambda.zero + Lambda %*% t(alpha * as.vector(pi)) %*% rep(1, K)
      cov <- (Gamma - Gamma %*% t(Si) %*% solve(diag(ni) + invvar %*% Si %*% Gamma %*%
                                                  t(Si)) %*% invvar %*% Si %*% Gamma) #/sigma
      etapred[ind,] <- mu + cov %*% t(Si) %*% invvar %*% (y - Si %*% mu)
      ord <- order( - pi)
      numb <- sum(cumsum(pi[ord]) <= tau1) + 1
      v <- diag(FFullS %*% (cov) %*% t(FFullS))
      pse <- sqrt(v + rep(sigma, rep(TT, J)))
      se <- sqrt(v)
      lower.p <- upper.p <- lower.c <- upper.c <- matrix(0, nrow(S)*J, numb)
      for(j in 1:numb) {
        mu <- lambda.zero + Lambda %*% alpha[ord[j],  ]
        mean <- FFullS %*% (mu + cov %*% t(Si) %*% invvar %*% (y - Si %*% mu))
      ...
      gpred[ind,] <- as.vector(FFullS %*%etapred[ind,])
      ind <- ind+1
    }
    meancurves <- FFullS%*%Lambda.alpha
...
"fclust.discrim.multicurve" <-
  function(fit,absvalue=F, scale=F){
    J = length(fit$data)
    S <- fit$FullS
    data = fit$data
    FFullS = lapply(1:J, function(j) {S})
    ...
    n <- nrow(S)
    Gamma <- fit$par$Gamma
    Sigma <- FFullS%*%Gamma%*%t(FFullS)+diag(rep(sigma, rep(n,J)))
    Lambda <- fit$par$Lambda
    discrim <- solve(Sigma)%*%FFullS%*%Lambda
    if (absvalue)
      discrim <- abs(discrim)
    n <- ncol(discrim)
    ...
    TT = length(fit$grid)
    ggrid = lapply(1:J, function(j) {fit$grid+fit$grid[TT]*(j-1)})
    ggrid = unlist(ggrid)
    if (scale) {
      for (j in 1:J) {
        idx = 1:TT + (j-1)*TT
        discrim[idx, ] = discrim[idx, ] * median(abs(data[[j]]$x), na.rm=T)
      }
    }
    for (i in 1:n){
      plot(ggrid,discrim[,i],ylab=paste("Discriminant Function ",i),xlab="Time",type='n')
      lines(ggrid,discrim[,i],lwd=3)
      abline(0,0)
      abline(v=c(1:(J-1))*fit$grid[TT], lty=2)
    }
  }

"fclust.plotcurves.multicurve" <-
  function(object=NULL,fit=NULL,index=NULL,ci=T,pi=T,clustermean=F, sep=F){
    if (is.null(object))
      object <- fclust.curvepred.multicurve(fit)
    if (is.null(index))
      index <- 1:length(table(object$data$curve))
    r <- ceiling(sqrt(length(index)))
    J = length(object$data)
    if (sep) par(mfrow=c(min(length(index),3),J))
    else par(mfrow=c(r,r))
    grid <- object$grid
    TT = length(grid)
    if (sep) {
      for (i in index){
        for (j in 1:J) {
          idx = 1:TT + (j-1)*TT
          upci <- object$upci[i,idx]
          uppi <- object$uppi[i,idx]
          lowci <- object$lowci[i,idx]
          lowpi <- object$lowpi[i,idx]
          gpred <- object$gpred[i,idx]
          meancurves <- (object$mean[idx,])
          if (clustermean)
          ...
          plot(grid,grid,ylim=yrange,ylab="Predictions",xlab="Time",type='n',
               main=paste("Curve ",i, "Functional Variable", j))
          if (clustermean)
            matpoints(grid, meancurves, type='b', col=6, lty=2, lwd=2, pch=NULL)
...
#               lines(grid,meancurves[,k],col=6,lty=2,lwd=2)
          ...
          lines(grid[object$data[[j]]$time[object$data[[j]]$curve==i]],object$data[[j]]$x[object$data[[j]]$curve==i],lwd=2)
          points(grid[object$data[[j]]$time[object$data[[j]]$curve==i]],object$data[[j]]$x[object$data[[j]]$curve==i],pch=19,cex=1.5)
        }
        
      }
    } else {
      ggrid = lapply(1:J, function(j) {grid+grid[TT]*(j-1)})
      ggrid = unlist(ggrid)
      for (i in index){
        upci <- object$upci[i,]
        uppi <- object$uppi[i,]
        ...
        if (clustermean)
          ...
          yrange <- c(min(lowpi),max(uppi))
        plot(ggrid,ggrid,ylim=yrange,ylab="Predictions",xlab="Time",type='n',
             main=paste("Curve ",i))
        if (clustermean)
          matpoints(ggrid, meancurves,type='b', col=6, lty=2, lwd=2)
#           for (k  in 1:ncol(meancurves))
#             lines(ggrid,meancurves[,k],col=6,lty=2,lwd=2)
        if (ci){
          lines(ggrid,upci,col=3)
          lines(ggrid,lowci,col=3)}
        if (pi){
          lines(ggrid,uppi,col=4)
          lines(ggrid,lowpi,col=4)}
        lines(ggrid,gpred,col=2,lwd=2)
        y = lapply(object$data, function(xx) {xx$x[xx$curve==i]})
        time = lapply(1:J, function(j) {object$data[[j]]$time[object$data[[j]]$curve==i] + (j-1)*TT})
        y = unlist(y)
        time = unlist(time)
        lines(ggrid[time],y,lwd=2)
        points(ggrid[time],y,pch=19,cex=1.5)
        abline(v=c(1:(J-1))*grid[TT], lty=2)
      }
    }
