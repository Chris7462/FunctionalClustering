...
for (j in 1:10) {
  ...
  curveData = list(makeCurveData(Data[[1]][idx,])$data, makeCurveData(Data[[2]][idx,])$data)
...
testProb = rep(NA, n) # probability of class 2 (fast progressor)
for (j in 1:nFold) {
  idx = unlist(fold[[j]])
  curveData = list(makeCurveData(Data[[1]][idx,])$data, makeCurveData(Data[[2]][idx,])$data)
  ...
  testProb[idx]=fclust.pred.multicurve(fclustFit[[j, k]], curveData)$probs[,fastProgressor]
}
