...
p <- ggplot(CL_S, aes(Nclust, fclustSil)) +
  geom_point(color = 'grey70', size=6) +
...
CL_T = readRDS(paste(RDSFigsPath,"PainL_Traj.rds",sep=''))
years <- c(0:7)
...
df= readRDS(paste(RDSFigsPath,"PainL_AUC1.rds",sep=''))
df2= readRDS(paste(RDSFigsPath,"PainL_AUC2.rds",sep=''))
df3= readRDS(paste(RDSFigsPath,"PainL_AUC3.rds",sep=''))
...
#__________________Plot Traj_______________________
CL_T = readRDS(paste(RDSFigsPath,"PainR_Traj.rds",sep=''))
years <- c(0:7)
dff <- data.frame(Type = rep(c("Nonprogressor", "Progressor"), each=8),
                  Year=years,
                  Progression=c(CL_T$Mean1, CL_T$Mean2))
...
#__________________Plot AUC_______________________
df= readRDS(paste(RDSFigsPath,"PainR_AUC1.rds",sep=''))
df2= readRDS(paste(RDSFigsPath,"PainR_AUC2.rds",sep=''))
df3= readRDS(paste(RDSFigsPath,"PainR_AUC3.rds",sep=''))
dff <- data.frame(Predictors = rep(c("YR 0", "YRS 0 to 1", "YRS 0 to 2"), each=1244),
                  Specificity=df$Specificity,
                  Sensitivity=c(df$Sensitivity,df2$Sensitivity, df3$Sensitivity))
p <- ggplot(dff, aes(Specificity, Sensitivity), color=Predictors) +
